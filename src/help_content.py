...
"""
    ...
    def get_normalization_description() -> str:
        ...
        return """🔬 РАСШИРЕННАЯ НОРМАЛИЗАЦИЯ

Включите чекбоксы для более точного сопоставления:

☑ Удалять юридические формы: ООО, Ltd, Inc, GmbH...
☑ Удалять версии: 2021, v4.x, R2, SP1, x64...
☑ Удалять стоп-слова: и, в, the, a...
...
Как работает:
  Объединяет значения из выбранных столбцов в одну строку
  Пример: "Microsoft" + "Office" = "Microsoft Office"

Рекомендации:
  • Включите ВСЕ опции нормализации для максимальной точности
  • Подходит для: vendor+product, категория+подкатегория
