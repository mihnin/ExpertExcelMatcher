...
**Обновлено:** 2025-10-22 (после рефакторинга)
**Размер итогового exe:** ~78 MB (оптимизированная сборка)
...
3. Размер файла:
   - Базовая сборка: ~90-100 MB
   - **Оптимизированная сборка: ~78 MB** ✅ (рекомендуется)
...
## ⚡ Оптимизированная сборка (РЕКОМЕНДУЕТСЯ)

### Использование оптимизированного spec файла

**После рефакторинга (2025-10-22)** доступна оптимизированная конфигурация:
...
**Преимущества:**
- ✅ Размер: ~78 MB (вместо ~100 MB)
- ✅ Исключены ненужные модули (matplotlib, scipy, jupyter, pytest)
- ✅ Совместимость с Python 3.13 + numpy
- ✅ UPX сжатие включено
- ✅ Все функции работают корректно

**Результат:**
- Файл: `dist/ExpertExcelMatcher.exe`
- Размер: 78 MB (81,838,330 байт)
- Запуск: успешный ✅

### Что исключено из сборки:
- matplotlib (графики не используются)
- scipy (научные вычисления не используются)
- jupyter/IPython (не используется)
- pytest (тесты не нужны в production)

## Дополнительная оптимизация

### Вариант 1: Уменьшение размера через excludes

```bash
pyinstaller --onefile --windowed --name "ExpertExcelMatcher" --exclude-module matplotlib --exclude-module scipy --exclude-module jupyter expert_matcher.py
```
...
## 📊 Результаты оптимизированной сборки (v2.2.0)

| Параметр | Базовая сборка | Оптимизированная |
|----------|----------------|------------------|
| **Размер exe** | ~90-100 MB | **78 MB** ✅ |
| **Время сборки** | ~2-3 мин | ~2-3 мин |
| **Python версия** | 3.13 | 3.13 |
| **Исключенные модули** | нет | matplotlib, scipy, jupyter, pytest |
| **UPX сжатие** | да | да |
| **Оптимизация байткода** | optimize=2 | optimize=0 (совместимость) |
| **Работоспособность** | ✅ | ✅ |

**Рекомендация:** Используйте `ExpertExcelMatcher_optimized.spec` для получения минимального размера при сохранении всей функциональности!
